...
function searchwiki() {
	$.ajax({
		url: ("https://en.wikipedia.org/w/api.php?format=json&action=query&generator=search&gsrsearch=" + searchItem + "&prop=extracts&exintro&explaintext&exsentences=1&exlimit=max&callback=?"),
		dataType: "json",
...
function listItems() {
	for (var i =0; i < res.length; i++) {
		$("ul").append("<li>" + "<span id = 'title'>" + "<a href = https://en.wikipedia.org/?curid=" + res[i].pageid + ">"
		+ res[i].title + "</a>" + "</span>" + "<br><br>" +  res[i].content + "</li>")
	}
}
